...
async fn handle_socket(socket: WebSocket, user: String, state: AppState) {
    ...
    let _ = tx.send(json!({ "type": "progress", "done": done, "total": total }).to_string()).await;
    ...
    match msg_type {
        "claim" => {
            ...
            match state.claim_file(file_id, user) {
                Ok(true) => {
                    state.broadcast(&json!({ "type": "presence", "user": user, "file_id": file_id }).to_string());
                }
                Ok(false) => {
                    let owner = state.claim_owner(file_id);
                    let _ = tx.send(
                        json!({ "type": "claim_rejected", "file_id": file_id, "owner": owner }).to_string(),
                    ).await;
                }
                Err(_) => {}
            }
        ...
        "release" => {
            ...
            if state.release_file(file_id, user) {
                state.broadcast(&json!({ "type": "release", "user": user, "file_id": file_id }).to_string());
            }
        }
        "annotate" => {
            let Some(file_id) = msg.get("file_id").and_then(|v| v.as_i64()) else {
            ...
            let lab = data.get("lab").and_then(|v| v.as_str()).unwrap_or("").to_string();
            let lab_without_tone = data
                .get("lab_without_tone")
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .to_string();
            let raw_text = data.get("raw_text").and_then(|v| v.as_str()).unwrap_or("").to_string();
