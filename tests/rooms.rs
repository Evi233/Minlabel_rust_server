...
async fn full_room_flow() {
    ...
    let form = reqwest::multipart::Form::new().text("user", "bob").part(
        "file",
        reqwest::multipart::Part::bytes(b"XXXX".to_vec()).file_name("b.wav"),
    );
    let resp = client
...
async fn rooms_are_isolated() {
    ...
    let room_a: Value = post(
        &client,
        &format!("{http}/api/rooms"),
        json!({ "user": "a" }),
    )
    .await;
    let room_b: Value = post(
        &client,
        &format!("{http}/api/rooms"),
        json!({ "user": "a" }),
    )
